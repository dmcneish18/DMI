...
## Without Names under the title Application title
   ...
   mainPanel(
     ...
     tags$style(HTML("
           .tabbable > .nav > li[class=active]    > a {background-color: #396b59; color:white}")),
     ...
     tabsetPanel(id="Tabs", type="pill",
       ...
       tabPanel( title = "Overview",
                 ...
                 h4(HTML("<li><b>This software simulates custom &#x0394RMSEA, &#x0394 CFI, &#x0394McDonald's Non-Centrality  cutoff values</b>")),
                  h4(HTML("<ul><li>Cutoffs are optimized for your specific model and data charactersitics")),
                  h4(HTML("<ul><li>The goal is to provide cutoffs that are optimal for a specific model and specific data")),
                  h4(HTML("<ul><li>Separate cutoffs are provided for metric, scalar, and strict invariance "), style = "margin-bottom: 30px;"),
                 ),
       #DMI Cutoff table
       ...
       tabPanel(title = "DMI Table",
                 ...
                 tags$details(tags$summary("Interpretation", style = "display: list-item;font-size:14px; font-weight:bold; ; margin-bottom:8px; width: 60%"),(
                  ...
                  tags$details(tags$summary("Purpose", style = "display: list-item;font-size:14px; font-weight:bold; ; margin-bottom:8px; width: 60%"),(
                   ...
                   <br>
                   <br>
                   However, behavior of fit indices is known to change as a function of the number of items, strict of loadings, distribution of responses, and missing data.
      ...
      ############# Dmacs Effect Sizes ##################
       ...
       tabPanel(title = "Non-Invariance Effect Sizes",
                h4(HTML("Your d<sub>MACS</sub> Effect Sizes:")),
                tableOutput("Dmacs"),
                p(HTML("<li>d<sub>MACS</sub> is intended for items that only load on a single factor. Results may not be correct for items with cross-loadings.
                        <li>d<sub>MACS</sub> uses a standard deviation pooled across groups and may be hard to interpret when items have very different variances in each group.")),
                ...
                tags$details(tags$summary("Interpretation", style = "display: list-item;font-size:14px; ; margin-bottom:8px; font-weight:bold;"),
                             p(HTML("d<sub>MACS</sub> is an effect size developed by Nye & Drasgow (2011)  to capture non-invariance in both loadings and intercepts across groups.
                             Its scale is similar to Cohen's <i>d</i> and values of .40, .60, and .80 correspond to small, medium, and large effects
                             of non-invariance (Nye et al., 2019).<br>
                             <br>
                             d<sub>MACS</sub> is intended to complement evaluations of the entire scale by helping to identify which items are potentially 
                             non-invariant and to what degree."), style="width:60%;")),
                tags$details(tags$summary("References", style = "display: list-item;font-size:14px; ; margin-bottom:8px; font-weight:bold;"),
                             p(HTML("Gunn, H. J., Grimm, K. J., & Edwards, M. C. (2020). Evaluation of six effect size measures of measurement 
                             non-invariance for continuous outcomes. <i>Structural Equation Modeling</i>, 27(4), 503-514.<br><br>
                             Nye, C. D., Bradburn, J., Olenick, J., Bialko, C., & Drasgow, F. (2019). 
                                    How big are my effects? Examining the magnitude of effect sizes in studies of 
                                    measurement equivalence. <i>Organizational Research Methods</i>, 22(3), 678-709. <br>
                                    <br>
                                    ...
                                    <i>Journal of Applied Psychology</i>, 96(5), 966-980.<br>
                                    <br>
                                    Somaraju, A. V., Nye, C. D., & Olenick, J. (2022). A review of measurement equivalence 
                                    in organizational research: What's old, what's new, what's next?. 
                                    <i>Organizational Research Methods</i>, 25(4), 741-785."), style="width:60%;"))
       ),
       ...
       tabPanel(title = "Path Diagram",
        ...
        tabPanel(title="FAQs",
                 ...
                 tags$details(tags$summary("Answer", style = "display: list-item;"),
                 p(HTML("<br>Computational times are a function of model size, sample size, the number of invariance types being tested, the estimator, and the precision.
                 As a rough guidline, models assessing all three types of invariance with 1000 people, 3 factors, continuous outcomes, maximum likelihood, and full precision take about 5-10 minutes. 
                ...
                <br>
                 ...
                 <br>
                 <br>
                 Cheung & Rensvold (2002) uses the perturbation method from Cudeck & Browne (1992)
                 ...
                 p(HTML("<br>")),
                 p(HTML("<b><li>What do I do if my scale does not meet the DMI cutoffs?</b>")),
                 tags$details(tags$summary("Answer", style = "display: list-item;"),
                              p(HTML("<br>DMI cutoffs are only one piece of evidence that can be used to evaluate meausurement invariance.
                                     The simulation that DMI cutoffs replicate intended to test that groups are within sampling error of invariant.
                                     Some consider this to be overly restrictive and that failing to meet this definition of invariance is not
                                     necessarily problematic.  <br>
                                     <br>
                                     In either case, it is helpful to explore which items are invariant to better understand why invariance may not have been upheld. 
                                     The 'Non-Invariance Effect Sizes' tab is provided to help further explore which items contributed to invariance. This tab
                                     provides a Cohen's <i>d</i> type effect size for each item to assess which items had to largest magnitude of non-invariance.
                                     More information about is provided under this tab. "), style="width:60%")),
       ...
       #references
        tabPanel(style="width:70%;",title = "References",
                 ...
                 p(HTML("McNeish, D. (in review). Dynamic measurement invariance cutoffs for two-group fit index differences.")),
                 p(HTML("<br>")),
                 ...
                 p(HTML("Dueber, D. (2020). dmacs: An R package for computing measurement nonequivalence indices.https://github.com/ddueber/dmacs.")),
  ...
  output$Group<-renderUI({
    req(input$upload)
      selectizeInput(inputId="Group",
                     ...
                     choices=names(data1()), multiple=TRUE, options=list(maxItems=1))
    })
...
###########################################
    ...
    #Fit model in lavaan
    a<-lavaan::cfa(data=data1(), model=model, estimator=input$est, orthogonal=COR, std.lv=TRUE, missing=FIML)
    ...
    #######################################
    if(input$est=="ML"){
    a1<-lavaan::cfa(data=data1(), model=model, estimator=input$est, group=input$Group,orthogonal=COR, std.lv=TRUE)
 ...
 ###############################
    ...
    output$Dmacs<-renderTable(D_new, rownames = T)
    ...
    if(input$Inv>0){
      fit2<-lavaan::cfa(data=data1(), model=model, estimator=input$est, orthogonal=COR, std.lv=TRUE, group=input$Group,group.equal="loadings")
    ...
    if(input$Inv>1){
      fit3<-lavaan::cfa(data=data1(), model=model, estimator=input$est, orthogonal=COR, std.lv=TRUE, group=input$Group,group.equal=c("loadings", "intercepts"))
    ...
    if(input$Inv>2){
      fit4<-lavaan::cfa(data=data1(), model=model, estimator=input$est, orthogonal=COR, std.lv=TRUE, group=input$Group,group.equal=c("loadings", "intercepts", "residuals"))
  ...
  ### Also deal with:
    ...
    true_fit_MI <- function(model,reps, n0, Scale=input$Scale){
      ...
      prop<-table(data1()[,input$Group])[1]/nrow(data1()[,input$Group])
    ...
    output$download <- downloadHandler(
     ...
     content = function(f) {
       ...
       rmarkdown::render("template-MI1.Rmd",
